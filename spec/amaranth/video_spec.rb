...
describe Amaranth::Video do
  describe ".all" do
    it "returns an array of hashes derived from Amara's API's 'objects' key" do
      stub_request(:get, "https://amara.org/api/videos/?team=complexity-explorer&limit=100")
        .to_return(body: <<-JSON)
    ...
    it "collates multiple pages" do
      stub_request(:get, "https://amara.org/api/videos/?team=complexity-explorer&limit=100")
        .to_return(body: <<-JSON)
          {
            "meta": {
              "next": "https://amara.org/api/videos/?offset=2&limit=2&team=complexity-explorer"
            },
            "objects": [{"id": 1},{"id": 2}]
          }
        JSON

      stub_request(:get, "https://amara.org/api/videos/?offset=2&limit=2&team=complexity-explorer")
        .to_return(body: <<-JSON)
          {
            "meta": {
              "next": null
    ...
    it "can narrow by project" do
      stub_request(:get, "https://amara.org/api/videos/?team=complexity-explorer&project=introduction-to-complexity&limit=100")
        .to_return(body: <<-JSON)
          { "meta": {}, "objects": [{"id": 1},{"id": 2}] }
        JSON
      described_class.all({
    ...
    describe ".create" do
      it "creates a video with attributes" do
        stub_request(:post, "https://amara.org/api/videos/")
    ...
    describe ".find_by_video_url" do
      it "returns a Video for the url if one is found" do
        ...
        video.should == Amaranth::Video.new({
          ...
          "team"=>nil,
          "project"=>nil,
          "all_urls" => ["https://youtube.com/watch?v=3f7l-Z4NF70"],
          "languages"=>[],
        })
      end
    end
    ...
    describe ".create_or_update_by_video_url" do
      it "updates an existing video if it exists" do
        ...
        stub_request(:put, "https://amara.org/api/videos/LrHZMMHioQHN/")
          .with(body: JSON.dump({
            "id":"LrHZMMHioQHN",
            "title":"updated title",
            "description":"updated description",
